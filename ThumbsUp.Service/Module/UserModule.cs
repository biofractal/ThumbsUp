	...
	public class UserModule : NancyModule
	{
			...
			: base("/user")
		{
			Post["/create"] = _ =>
			{
				...
				if (!userService.ValidateUserName(username)) return Error.Generate(Response, 3);
			...
			Post["/get"] = url =>
			{
				...
				return (user == null) ? Error.Generate(Response, 2) : Response.AsJson(new { User = new { Id = user.Id, UserName = user.UserName, Email = user.Email } });
			};
			...
			Post["/validate"] = _ =>
			{
				...
				return (key == null) ? Error.Generate(Response, 1) : Response.AsJson( new { ThumbKey = key });
			};
			...
			Post["/validate/thumbkey"] = _ =>
			{
				...
				return !isValid ? Error.Generate(Response, 4) : HttpStatusCode.OK;
			};
			...
			Post["/validate/name"] = _ =>
			{
				...
				return !isValid ? Error.Generate(Response, 5) : HttpStatusCode.OK;
			};
			...
			Post["/logout"] = _ =>
			{
				...
				return !success ? Error.Generate(Response, 4) : HttpStatusCode.OK;
			};
