...
namespace ThumbsUp.UnitTest.Tests
{
	public class UserValidate : _BaseTest
	{
		...
		public void Should_return_thumbkey_when_valid_user_credentials_are_supplied()
		{
			...
			A.CallTo(() => mockUserService.ValidateUser(A<string>.Ignored, A<string>.Ignored)).Returns(new Guid().ToString());
		...
		[Fact]
		public void Should_return_NoUserForCredentials_error_when_invalid_user_credentials_are_supplied()
		{
			// Given
			var mockUserService = A.Fake<IUserService>();
			A.CallTo(() => mockUserService.ValidateUser(A<string>.Ignored, A<string>.Ignored)).Returns(null);
			var userTestBrowser = MakeTestBrowser<UserModule>(mockUserService: mockUserService);
			...
			{
				with.HttpRequest();
				with.FormValue("username", "<invalid-username>");
				with.FormValue("password", "<invalid-password>");
			});
